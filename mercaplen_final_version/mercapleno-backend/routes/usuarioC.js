...
//  CORRECCIÓN: Desestructuramos la función verifyToken del objeto exportado
const { verifyToken } = require("./authMiddleware"); 

const bcrypt = require("bcryptjs");

// Aplica el middleware de autenticación a todas las rutas CRUD
// CORRECCIÓN: Usamos la función desestructurada
...
router.post("/", async (req, res) => {
  ...
  try {
    await query(sql, [nombre, apellido, email, hashedPassword, direccion, fecha_nacimiento, id_rol, id_tipo_identificacion, numero_identificacion]);
...
router.put("/:id", async (req, res) => {
  const { id } = req.params;
  const { nombre, apellido, email, direccion, fecha_nacimiento, id_rol, id_tipo_identificacion, numero_identificacion, password } = req.body;
